...
internal class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        // upravial jsem si zadání, slovník obsahuje vždy položka: (cena za kus, počet kusů), aby šlo upravovat cenu i pokud by se přidávalo víc než jeden kus :D

        AddItemToShoppingList(shopping, "chleba");
        AddItemToShoppingList(shopping, "banán");
        AddItemToShoppingList(shopping, "bonbon");

        Console.WriteLine("Částky za jednotlivé položky z nákupní tašky");
        foreach ((string item, int[] priceAndPieces) in shopping)
        {
            Console.WriteLine($"{item}: cena celkem: {priceAndPieces[0] * priceAndPieces[1]} (počet kusů: {priceAndPieces[1]})");
        }
    }
}
